...
function assertInstalled() {
  modsAndWebsite.forEach((repo) => {
    if (!fs.existsSync(repo)) {
      console.error(
        "module `%s` is missing. Did you forget to run `pm install`?",
        repo
      );
      process.exit(1);
    }
...
function lint() {
  ...
  require("glob")
    .sync("website/pages/examples/*/example.js")
    .forEach((file) => blint.checkFile(file, websiteOptions));
}
...
function install(arg = null) {
  ...
  if (arg == "--ssh") {
    base = "git@github.com:ProseMirror/";
  } else if (arg != null) help(1);

  console.log("set git proxy");
  ...
  try {
    modsAndWebsite.forEach((repo) => {
      ...
      let origin =
        base + (repo == "website" ? "" : "prosemirror-") + repo + ".git";
      run("git", ["clone", origin, repo]);
    });
  } finally {
    console.log("reset git proxy");
...
function grep(pattern) {
  ...
  mods.forEach((repo) => {
    files = files
      .concat(glob.sync(repo + "/src/*.js"))
      .concat(glob.sync(repo + "/test/*.js"));
  });
  files = files
    .concat(glob.sync("website/src/**/*.js"))
    .concat(glob.sync("website/pages/examples/*/*.js"));
  try {
    console.log(
      run(
        "grep",
        ["--color", "-nH", "-e", pattern].concat(
          files.map((f) => path.relative(origDir, f))
        ),
        origDir
      )
    );
...
function release(mod) {
  ...
  fs.writeFileSync(
    mod + "/CHANGELOG.md",
    notes.head + notes.body + fs.readFileSync(mod + "/CHANGELOG.md", "utf8")
  );
...
function changelog(repo, since) {
  let commits = run(
    "git",
    ["log", "--format=%B", "--reverse", since + "..master"],
    repo
  );
  ...
  while ((match = re.exec(commits)))
    result[match[1].toLowerCase()].push(match[2].replace(/\r?\n/g, " "));
  return result;
}
...
function releaseNotes(mod, changes, version) {
  ...
  for (let type in types) {
    ...
    messages.forEach(
      (message) =>
        (body += message.replace(/\]\(##/g, "](" + refTarget + "#") + "\n\n")
    );
  }
  return { head, body };
}
...
function setDepVersion(mod, version) {
  modsAndWebsite.forEach((repo) => {
    ...
    if (result != text) {
      ...
      run(
        "git",
        ["commit", "-m", `Upgrade prosemirror-${mod} dependency`],
        repo
      );
    }
  });
...
function watch() {
  ...
  let watcher = watch(configs),
    cwd = process.cwd();
  function name(input) {
    return input.slice(cwd.length + 1).match(/[^\/]*/)[0];
  }
  ...
  modsAndWebsite.forEach((repo) => {
    ...
    glob.sync(repo + "/" + file).forEach((file) => {
      let content = fs.readFileSync(file, "utf8"),
        changed = content.replace(re, replacement);
      if (changed != content) {
